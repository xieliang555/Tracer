...
class PegInHole(gym.Env):
	def __init__(self, isGUI, holeType, seed=1):
		...
		# !!!!!
		# dx, dy, dtheta
		self.action_space = gym.spaces.Box(low=np.float32(-2.0),high=np.float32(2.0), shape=(3,), dtype=np.float32)
	...
	def reset(self):
		...
		self.offset_translate = 0.001*np.random.uniform(-2,2,2)
		self.offset_rotation = np.random.uniform(-2,2,1)[0]
	...
	def step(self, a):
		# a: [dx(mm), dy(mm), dyaw(deg)] in the current ft/endEffector coordinate system
		self.stepCount += 1

		self.offset_translate += np.array(a[0:2])*0.001
		self.offset_rotation += a[2]
		pose = self.transform(
			R.from_euler('z', self.offset_rotation, degrees=True).as_matrix(),
			[self.offset_translate[0],self.offset_translate[1],0])
		...
		if encoder_gt[0]>20 or encoder_gt[0]<0 or encoder_gt[1]>20 or encoder_gt[1]<0:
			self.d = True
		if self.offset_rotation > 2 or self.offset_rotation<-2:
			self.d = True
		if abs(encoder_gt[0]-10)<5 and abs(encoder_gt[1]-10)<5 and abs(self.offset_rotation)<=1:
			self.d = True
