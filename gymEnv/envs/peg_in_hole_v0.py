...
class PegInHole(gym.Env):
	...
	def hybrid_force_pos_control(self, target_pos, num_steps=100):
		...
		for t in range(num_steps):
			...
			f_wrench = R.from_euler('x', 180, degrees=True).apply(ft_transform[0:3])
			t_wrench = R.from_euler('x', 180, degrees=True).apply(ft_transform[3:])
			ft_wrench = np.concatenate((f_wrench, t_wrench), 0)
			
			force_err = pid.calc(ft_wrench[2], setpoint)
	...
	def reset(self):
		...
		t_wrench = R.from_euler('x', 180, degrees=True).apply(ft_transform[3:])
		ft_wrench = np.concatenate((f_wrench, t_wrench), 0)
		# print('after', ft_wrench)

		self.cur_pos, self.cur_orn = p.getLinkState(self.panda_peg_id, self.ftJointIndex)[0:2]
	...
	def step(self, a):
		...
		if a == 0:
			self.offset += [0.001, 0, 0]
		elif a == 1:
			self.offset += [-0.001, 0, 0]
		elif a == 2:
			self.offset += [0, -0.001, 0]
		elif a == 3:
			self.offset += [0, 0.001, 0]

		target_pos = R.from_quat(self.cur_orn).apply(R.from_euler('x', 180, degrees=True).apply(self.offset))+self.cur_pos
		...
		info = {}
		return ft_wrench, reward, self.done, info
