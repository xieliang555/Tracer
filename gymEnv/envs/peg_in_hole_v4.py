...
class PegInHole(gym.Env):
	...
	def visualize(self, dx=0, dy=0, logits=0, dense_map_self_atten=0, sparse_map_self_atten=0):
		...
		plt.imshow(logits[0].reshape(21,21), cmap='gray')
		# ax.axes.xaxis.set_ticks([])
		# ax.axes.yaxis.set_ticks([])
		ax.set_xticks([0,5,10,15])
		ax.set_yticks([0,5,10,15,20])
		...
		plt.imshow(self.dense_map_reduced)
		ax.scatter(dy,dx, 50, c='w', marker='+')
		# ax.axes.xaxis.set_ticks([])
		# ax.axes.yaxis.set_ticks([])
		...
		# # dense map with self attention
		# ax = plt.subplot(2,3,4)
		# ax.set_title('4.self-attention')
		# # ax.set_ylabel('Attention map')
		# dense_map_pca = self.pca_1.fit_transform(dense_map_self_atten[0])
		# dense_map_pca = (dense_map_pca-dense_map_pca.min())/(dense_map_pca.max()-dense_map_pca.min())
		# plt.imshow(dense_map_pca.reshape(21,21), cmap='gray')
		# # ax.axes.xaxis.set_ticks([])
		# # ax.axes.yaxis.set_ticks([])
		# ax.set_xticks([0,5,10,15])
		# ax.set_yticks([0,5,10,15,20])

		# # sparse map with self attention
		# ax = plt.subplot(2,3,5)
		# ax.set_title('5.self-attention')
		# sparse_map_pca = self.pca_1.fit_transform(sparse_map_self_atten[0])
		# sparse_map_pca = (sparse_map_pca-sparse_map_pca.min())/(sparse_map_pca.max()-sparse_map_pca.min())
		# plt.imshow(sparse_map_pca.reshape(21,21), cmap='gray')
		# # ax.axes.xaxis.set_ticks([])
		# # ax.axes.yaxis.set_ticks([])
		# ax.set_xticks([0,5,10,15])
		# ax.set_yticks([0,5,10,15,20])
		...
		# # ft = self.pca.transform(self.ft.reshape(-1,6))
	...
	def reset(self):
		...
		# !!!!!!!!!!!!!1
		self.maxStep = 20
		self.sparse_map = np.zeros((441,6), dtype=np.float32)
		self.reset_dxy = [0,0]
	...
	def step(self, a):
		...
		return self.ft, self.dense_map, self.sparse_map, self.dense_index, self.sparse_index, self.stepCount, self.d
